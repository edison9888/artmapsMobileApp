...
@property (nonatomic, retain) OoIMetaGLA *selectedOoiMetaGLA;
...
-(void)OoISearchLoader:(OoISearchLoader *)loader didFailWithError:(NSError *)error{
    ooiSearchLoader.delegate = nil;
    [UIApplication sharedApplication].networkActivityIndicatorVisible = NO;
    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:self.title message:NSLocalizedString(@"Application can't download data. Please check your internet connection", @"") delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
    [alertView show];
    [alertView release];
}
...
-(void)submitOoIMetaRequestForAnnotation:(OoILocation*)location{
    ooiMetaLoader.delegate = self;
    ooiMetaLoader.refObjID = [[[NSNumber alloc]initWithInt:location.location_id] autorelease];
...
-(void)showMetaTableViewControllerForAnnotation:(OoILocation*)annotation meta:(OoIMeta*)ooiMeta{
    ...
    if(title == @"TATE - Art Maps"){
        ...
        metaTableViewController.ooiID = annotation.parent.OoI_id;
        ...
        MetaDataGLAViewController *metaTableGLAViewController = [[MetaDataGLAViewController alloc] initWithNibName:@"MetaDataGLAViewController" bundle:nil];
        metaTableGLAViewController.ooiID = annotation.parent.OoI_id;
        metaTableGLAViewController.ooiMeta = selectedOoiMetaGLA;
        metaTableGLAViewController.postMapLocation = [self getPostMapLocationForAnnotation:annotation];
        [self.navigationController pushViewController:metaTableGLAViewController animated:YES];
        [metaTableGLAViewController release];
      }
}
...
-(void)OoIMetaLoader:(OoIMetaLoader *)loader didLoadOoIMeta:(OoIMeta *)ooiMeta{
    ...
    if (location != nil){
        ...
        if(title == @"TATE - Art Maps"){
            ...
            [self setAnnotationViewLabel:view text:ooiMeta.allegiance];
            self.selectedOoiMeta = ooiMeta;
        }*/
    }
}
...
//#pragma mark - OoIMetaLoaderDelegate
-(void)OoIMetaLoader:(OoIMetaLoader *)loader didLoadOoIMetaGLA:(OoIMetaGLA *)ooiMetaGLA{
    ooiMetaLoader.delegate = nil;
    OoILocation* location = [self findLocationAnnotationInMapView:mapViewControl objID:loader.refObjID];
    if (location != nil){
        OoIAnnotationView *view = (OoIAnnotationView*)[mapViewControl viewForAnnotation:location];
        [self setAnnotationViewLabel:view text:ooiMetaGLA.name];
        self.selectedOoiMetaGLA = ooiMetaGLA;
    }
}
...
- (void)locationManager:(CLLocationManager *)manager didUpdateToLocation:(CLLocation *)newLocation fromLocation:(CLLocation *)oldLocation {
    ...
    if (oldLocation == nil) {
        ...
        MKCoordinateRegion userLocation = MKCoordinateRegionMakeWithDistance(newLocation.coordinate, 4500.0, 4500.0);
